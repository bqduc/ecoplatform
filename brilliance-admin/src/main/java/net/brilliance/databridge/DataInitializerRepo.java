...
public class DataInitializerRepo {
	...
	public static AddressPart[] buildAddresses() {
		...
		List<AddressPart> addresses = ListUtility.createArrayList();
		...
		for (int i = 0; i < NUMBER_TO_GENERATE; ++i) {
			addresses.add(
					...
					.primary(faker.address().fullAddress())
					.stateProvince(faker.address().cityName())
					.postalCode(faker.address().zipCode()).build());
		}
		return addresses.toArray(new AddressPart[0]);
	}
