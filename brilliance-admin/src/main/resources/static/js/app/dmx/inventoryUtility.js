...
$(document).ready( function () {
	...
	$('#app-locale-english, #app-locale-english-info').click(function (){
	    ...
	    dataTableObject = $('#inventoryEntryTable').dataTable( {"url": enLanguageUrl} );
	});
	...
	$('#app-locale-vietnamese, #app-locale-vietnamese-info').click(function (){
	    ...
	    dataTableObject = $('#inventoryEntryTable').dataTable( {"url": vnLanguageUrl} );
	});
	...
	console.log('COME');
	dataTableObject = $('#'+ workObjectId + 'Table').dataTable({
	 	"language": {
	 		"url": currentLanguageUrl
     	},
     	"pageLength":"50",
		"sAjaxSource": "/rapi/inventory/list",
		"sAjaxDataProp": "",
		"order": [[ 0, "asc" ]],
		"aoColumns": [
		      { "mData": "id", "width": "2%"},
	...
	$('#searchObjects').on('click', function(e){
		...
		refreshTable('#' + workObjectId + 'Table', '/employee/searchEmployee?size='+size+'&page='+page);
	});
});